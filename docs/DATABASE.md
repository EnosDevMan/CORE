...
Novos avatares usam `professionals/<id>-<sufixo>`. A policy aceita também
`barbers/<id>-<sufixo>` durante deploys graduais e para objetos legados, sempre
exigindo que `<id>` corresponda ao `profile_id` da sessão profissional.
