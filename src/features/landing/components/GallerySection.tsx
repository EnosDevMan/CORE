...
export const GallerySection: React.FC<{ galleryPhotos: GalleryPhoto[]; config: BusinessConfig }> = ({ galleryPhotos, config }) => {
 ...
 return <section id="gallery-section" className="bg-brand-navy px-4 py-14 sm:py-16 lg:py-20"><div className="mx-auto max-w-6xl">
  ...
  <div className="mt-7 grid grid-cols-2 grid-rows-2 gap-2 sm:grid-cols-4 sm:gap-3">{galleryPhotos.slice(0, 5).map((photo, index) => <figure key={photo.id} className={`relative overflow-hidden bg-slate-100 ${index === 0 ? 'col-span-2 row-span-2 aspect-[4/3] sm:aspect-auto' : 'aspect-square'}`}><img src={photo.imageUrl} alt={photo.caption || 'Trabalho realizado pelo estabelecimento'} width="640" height="640" loading="lazy" className="h-full w-full object-cover"/>{photo.caption && <figcaption className="absolute inset-x-0 bottom-0 bg-black/65 p-2 text-xs text-white">{photo.caption}</figcaption>}</figure>)}</div>
