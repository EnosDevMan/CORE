...
export const ServicesSection: React.FC<Props> = ({ categories, activeServices, onSelectService }) => {
  ...
  return <section id="services-section" className="core-public-page px-4 py-14 sm:py-16 lg:py-20">
    <div className="mx-auto max-w-6xl">
      ...
      <div ref={rail} className="services-rail -mx-4 mt-6 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-3" aria-live="polite">
        {filtered.map(service => <article key={service.id} className="service-card core-public-elevated core-public-border flex shrink-0 snap-start flex-col border p-5">
          ...
          <button onClick={() => onSelectService(service.id)} className="core-public-primary core-public-ring mt-5 min-h-12 w-full px-4 py-3 text-sm font-bold transition-opacity hover:opacity-90">Escolher serviço</button>
        </article>)}
      </div>
