...
export function OnboardingWizard({ currentRole }: { currentRole: UserRole }) {
  ...
  const [paletteId, setPaletteId] = useState(NICHE_REGISTRY.barbershop.defaultPaletteId);
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      await onboardingService.complete({ businessName, nicheId, themeStyleId, paletteId, phone, address,
  ...
  return <main className="min-h-screen bg-slate-950 px-4 py-6 text-slate-100 sm:py-12"><form onSubmit={submit} className="mx-auto flex min-h-[calc(100vh-3rem)] w-full max-w-xl flex-col rounded-3xl border border-white/10 bg-slate-900 p-5 shadow-2xl sm:min-h-0 sm:p-8">
    ...
    <section className="flex-1">
      ...
      {step==='theme' && <div><h2 className="mb-4 text-lg font-bold">Escolha a aparência do site</h2><AppearancePicker nicheId={nicheId} styleId={themeStyleId} paletteId={paletteId} onStyleChange={setThemeStyleId} onPaletteChange={setPaletteId} tone="dark" /></div>}
      ...
      {step==='review' && <div><h2 className="text-lg font-bold">Revise a instalação</h2><dl className="mt-4 divide-y divide-slate-700 rounded-2xl border border-slate-700 px-4">{[['Negócio',businessName],['Nicho',niche.name],['Estilo',THEME_STYLE_REGISTRY[themeStyleId].name],['Cores',PALETTE_REGISTRY[paletteId].name],['Horário',`${open}–${close}`],['Serviços',String(services.filter(s=>s.name.trim()).length)],['Equipe',String(professionals.filter(p=>p.name.trim()).length)]].map(([term,value])=><div className="py-3" key={term}><dt className="text-xs text-slate-400">{term}</dt><dd className="font-bold">{value}</dd></div>)}</dl>{error&&<p role="alert" className="mt-4 rounded-xl bg-red-950 p-3 text-sm text-red-200">{error}</p>}</div>}
    </section>
