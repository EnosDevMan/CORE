...
const suggestedServices = (nicheId: NicheId) => NICHE_REGISTRY[nicheId].serviceSuggestions
  .map(item => ({ ...item, price: '0,00' }));
...
export function OnboardingWizard({ currentRole }: { currentRole: UserRole }) {
  ...
  const [services, setServices] = useState(() => suggestedServices('barbershop'));
  ...
  const allValid = validBusiness && validHours && validServices && validProfessionals && validBookingRules;
  ...
  return <main className="min-h-screen bg-slate-950 px-4 py-6 text-slate-100 sm:py-12"><form onSubmit={submit} className="mx-auto flex min-h-[calc(100vh-3rem)] w-full max-w-xl flex-col rounded-3xl border border-white/10 bg-slate-900 p-5 shadow-2xl sm:min-h-0 sm:p-8">
    ...
    <section className="flex-1">
      ...
      {step==='theme' && <fieldset><legend className="mb-4 text-lg font-bold">Escolha uma identidade visual</legend><div className="grid gap-3 sm:grid-cols-2">{themes.map(item=><label key={item.id} className={`flex cursor-pointer items-center gap-3 rounded-2xl border p-4 ${themeId===item.id?'border-amber-400':'border-slate-700'}`}><input className="sr-only" type="radio" name="theme" checked={themeId===item.id} onChange={()=>setThemeId(item.id)} /><span className="h-10 w-10 rounded-full" style={{background:item.tokens.primary}} /><strong>{item.name}</strong>{themeId===item.id&&<Check className="ml-auto text-amber-400" />}</label>)}</div></fieldset>}
      ...
      {step==='services' && <div><h2 className="text-lg font-bold">Serviços iniciais</h2><p className="mb-4 text-sm text-slate-400">Sugestões do nicho; informe os preços, edite ou remova livremente. Valor zero será exibido como serviço gratuito.</p><div className="space-y-3">{services.map((item,index)=><div key={index} className="grid grid-cols-[minmax(0,1fr)_88px_44px] gap-2 sm:grid-cols-[minmax(0,1fr)_90px_110px_44px]"><input aria-label={`Serviço ${index+1}`} maxLength={100} className="ui-input col-span-3 sm:col-span-1" value={item.name} onChange={e=>setServices(current=>current.map((value,i)=>i===index?{...value,name:e.target.value}:value))} /><input aria-label={`Duração do serviço ${index+1} em minutos`} title="Duração em minutos" type="number" step={1} min={5} max={480} className="ui-input" value={item.duration} onChange={e=>setServices(current=>current.map((value,i)=>i===index?{...value,duration:Number(e.target.value)}:value))} /><input aria-label={`Preço do serviço ${index+1} em reais`} title="Preço em reais" inputMode="decimal" maxLength={16} placeholder="R$ 0,00" className="ui-input" value={item.price} onChange={e=>setServices(current=>current.map((value,i)=>i===index?{...value,price:e.target.value}:value))} /><Button variant="ghost" aria-label={`Remover ${item.name || `serviço ${index+1}`}`} onClick={()=>setServices(current=>current.filter((_,i)=>i!==index))}><Trash2 size={18}/></Button></div>)}</div>{!validServices&&<p role="alert" className="mt-3 text-sm text-red-300">Nomes preenchidos devem ter de 2 a 100 caracteres, duração inteira de 5 a 480 minutos e preço válido não negativo.</p>}<Button variant="secondary" className="mt-3" onClick={()=>setServices(current=>[...current,{name:'',duration:30,price:'0,00',category:''}])}><Plus size={18}/> Adicionar</Button></div>}
