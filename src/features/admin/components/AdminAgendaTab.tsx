...
export const AdminAgendaTab: React.FC<AdminAgendaTabProps> = ({ showFeedback }) => {
  ...
  // corresponda ao dia real de funcionamento do estabelecimento.
  ...
  const getProfessionalName = (id: string) => professionalNames.get(id) ?? getSharedProfessionalName(professionals, id);
  const getServiceName = (id: string) => {
    if (!id.includes(',')) return serviceNames.get(id) ?? getSharedServiceName(services, id);
    return getSharedServiceName(services, id);
  };
  ...
  const filteredBookings = useMemo(() => {
    ...
    return bookings
      .filter(booking => {
        if (booking.date < startDate || booking.date > endDate) return false;
        if (professionalFilter !== 'all' && booking.professionalId !== professionalFilter) return false;
        if (statusFilter !== 'all' && booking.status !== statusFilter) return false;
        if (!query) return true;
        return `${booking.customerName} ${booking.customerPhone} ${getServiceName(booking.serviceId)}`
          .toLocaleLowerCase('pt-BR')
          .includes(query);
      })
      .sort((a, b) => `${a.date}${a.time}`.localeCompare(`${b.date}${b.time}`));
  }, [bookings, dateFilter, period, professionalFilter, search, statusFilter, services, serviceNames]);
        ...
        <div className="divide-y divide-slate-100 max-h-[600px] overflow-y-auto custom-scrollbar">
          {filteredBookings.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <p className="text-slate-500 text-sm">Não há horários marcados para os filtros selecionados.</p>
            </div>
          ) : (
