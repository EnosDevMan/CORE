...
export const useAppStore = () => {
   ...
   * conflitos de agendamento (condição de corrida) acontece no PostgreSQL,
   * por RPC transacional, trigger e constraint de exclusão.
   */
  ...
  const getAvailabilitySlots = useCallback((professionalId: string, serviceId: string, date: string, includeElapsed = false, excludeBookingId?: string, occupiedIntervals: { time: string; duration: number }[] = [], durationSnapshot?: number) => {
   ...
   */
  const getAvailableSlots = useCallback(async (professionalId: string, serviceId: string, date: string, excludeBookingId?: string, durationSnapshot?: number): Promise<string[]> => {
    ...
    const slots = getAvailabilitySlots(professionalId, serviceId, date, false, excludeBookingId, occupiedIntervals, durationSnapshot);
