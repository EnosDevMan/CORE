...
-- O namespace barbers permanece temporariamente para objetos antigos.
create policy "avatars_professional_insert_own" on storage.objects for insert
  with check (
    bucket_id = 'avatars'
    and auth_role() = 'professional'
    and (
      name like 'professionals/' || (select profile_id::text from profiles where id = auth.uid()) || '-%'
      or name like 'barbers/' || (select profile_id::text from profiles where id = auth.uid()) || '-%'
    )
  );
